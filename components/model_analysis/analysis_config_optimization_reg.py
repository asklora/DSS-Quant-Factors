...
if __name__ == "__main__":
    ...
    if type(args.name_sql) == type(None):
        ...
        query = f"SELECT * FROM {tbl_name} WHERE _name_sql='{args.name_sql}'"
    ...
    # df_pillar = df.loc[
    #     (~df['_name_sql'].isin(['w4_d-7_20220324031027_debug', 'w4_d-7_20220329120327_debug'])) &
    #     (df['_q'].astype(float) == 0.33),
    ...
    df['_name_sql'] = df['_name_sql'].str[:2]
    df = df.groupby(['_name_sql', '_train_currency', '_pred_currency', '_testing_period', '_pillar', '_eval_q',
                     '_eval_removed_subpillar']).mean().reset_index()
    ...
    df['_train_currency'] = np.where(df['_pred_currency'] == df['_train_currency'], 'own', 'usd')
    df["_pillar"] = np.where(df['_pillar'].str.startswith('pillar'), 'cluster', df['_pillar'])

    df_pivot = df.groupby(['_name_sql', '_testing_period', '_pillar',
                           '_pred_currency', '_train_currency'])[['max_ret', 'min_ret']].mean().unstack()
    ...
    # df_pivot = df_pivot.merge(df_index, left_on=['_testing_period', '_train_currency'], right_index=True, how='left')
    # df_pivot['actual1'] = df_pivot.groupby(['_train_currency', '_pillar'])['actual'].transform('mean')
    ...
    df_pivot_count = df_pivot.groupby(['_name_sql', '_pred_currency', '_pillar'])['best'].apply(lambda x: dict(Counter(x)))
    ...
    df_pivot_avg = df_pivot.groupby(['_name_sql', '_pred_currency', '_pillar'])[ret_cols].mean().stack().rename("mean")
    df_pivot_std = df_pivot.groupby(['_name_sql', '_pred_currency', '_pillar'])[ret_cols].std().stack().rename("std")
    df_pivot_min = df_pivot.groupby(['_name_sql', '_pred_currency', '_pillar'])[ret_cols].min().stack().rename("min")
    df_pivot_quantile = df_pivot.groupby(['_name_sql', '_pred_currency', '_pillar'])[ret_cols].quantile(q=0.05).stack().rename("q5")
    df_pivot_quantile1 = df_pivot.groupby(['_name_sql', '_pred_currency', '_pillar'])[ret_cols].quantile(q=0.1).stack().rename("q10")
    df_pivot_sort0 = df_pivot.groupby(['_name_sql', '_pred_currency', '_pillar']).apply(lambda x: sortino(x[ret_cols], actual_col=0)).stack().rename("sortino_0")
    # df_pivot_sort = df_pivot.groupby(['_name_sql', '_pred_currency', '_pillar']).apply(lambda x: sortino(x[ret_cols], actual_col='actual')).stack().rename("sortino")
    # df_pivot_sort1 = df_pivot.groupby(['_name_sql', '_pred_currency', '_pillar']).apply(lambda x: sortino(x[ret_cols], actual_col='actual1')).stack().rename("sortino_ts")
    df_pivot_sharpe0 = df_pivot.groupby(['_name_sql', '_pred_currency', '_pillar']).apply(lambda x: sharpe(x[ret_cols], actual_col=0)).stack().rename("sharpe_0")
    # df_pivot_sharpe = df_pivot.groupby(['_name_sql', '_pred_currency', '_pillar']).apply(lambda x: sharpe(x[ret_cols], actual_col='actual')).stack().rename("sharpe")
    # df_pivot_sharpe1 = df_pivot.groupby(['_name_sql', '_pred_currency', '_pillar']).apply(lambda x: sharpe(x[ret_cols], actual_col='actual1')).stack().rename("sharpe_ts")
    ...
    xlsx_dict = {'raw': df_pivot_eval.sort_values(by=['_name_sql', '_pred_currency', '_pillar', 'sortino_0'],
                                                  ascending=False).groupby(['_name_sql', '_pred_currency', '_pillar']).head(20)}
    for name, g in df_pivot_eval.groupby(['_name_sql', '_pred_currency']):
        xlsx_dict[','.join(list(name))] = g.groupby(['_pillar', 'level_3'])[['mean', 'sortino_0', 'min']].mean().stack().unstack(level=1).reset_index()
    # for i in ['mean', 'sortino_0', 'min']:
    #     xlsx_dict[i] = df_pivot_eval.groupby(['_name_sql', '_pred_currency', '_pillar', 'level_3'])[i].mean().unstack().reset_index()
