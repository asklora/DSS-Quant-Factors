...
class clusterFeature:
    ...
    def __init__(self, df):
        ...
        self.agglo = FeatureAgglomeration(n_clusters=None, distance_threshold=0,
                                          linkage='average')
    ...
    def find_subpillar(self, subpillar_trh):
        ...
        subpillar_cluster = FeatureAgglomeration(n_clusters=None,
                                                 distance_threshold=subpillar_dist,
                                                 linkage='average')
        subpillar_cluster.fit(self.X)
        subpillar_label = subpillar_cluster.labels_
        logger.debug(Counter(subpillar_label))
        subpillar = {
            f"subpillar_{k}": list(self.feature_names[subpillar_label == k]) for
            k, v in
    ...
    def find_pillar(self, pillar_trh):
        ...
        pillar_cluster = FeatureAgglomeration(n_clusters=None,
                                              distance_threshold=pillar_dist,
                                              linkage='average')
        pillar_cluster.fit(self.X)
        pillar_label = pillar_cluster.labels_
        logger.debug(Counter(pillar_label))
        pillar = {f"pillar_{k}": list(self.feature_names[pillar_label == k]) for
                  k, v in
...
class CalcPillarCluster:
    """
    calculate pillar / subpillar using hierarchical clustering on ratios over
    past (n = self.lookback) years
    - pillar: factors will be evaluated together (i.e. equivalent to
        value/momentum/quality) in multi-output RF tree
    - subpillar: factors will not be selected together if both deemed as
        "good" / "bad" factors
    """

    active_factor = read_query_list(
        f"SELECT name FROM {factors_formula_table} WHERE is_active")
    ...
    def __init__(self, weeks_to_expire: int, sample_interval: int,
                 ...
                 subpillar_trh: int = 5, pillar_trh: int = 2, lookback: int = 5,
                 processes: int = 1):
        """
        Parameters
        ----------
        subpillar_trh (Int):
            nth smallest distances below which factors belong to same sub-pillar
             (i.e. factors won't be selected together)
        pillar_trh (Int):
            nth largest distances below which factors belong to same pillar
        lookback (Int):
            number of years for sample lookback periods prior to testing_period
            for clustering
        """

        self.period_list = self._testing_period_list(start_date, end_date,
                                                     sample_interval,
                                                     weeks_to_expire)
    ...
    def _testing_period_list(self, start_date, end_date, sample_interval,
                             weeks_to_expire):
        """
        Data processing: align period_list Timestamp to match premium table
        i.e. testing_period = (last_data_date - weeks_to_expire)
        """
        ...
        date_list = pd.date_range(end=end_date, freq=f"{sample_interval}W-SUN",
                                  periods=periods)
        period_list = list(
            [x - relativedelta(weeks=weeks_to_expire) for x in date_list])
    ...
    def write_all(self):
        ...
        with mp.Pool(processes=self.processes,
            ...
            results = pool.starmap(partial(self._calc_cluster, ratio_df=df),
                                   all_groups)

        results = pd.concat([x for x in results if type(x) != type(None)],
                            axis=0)
        ...
        logger.info(
            f"=== Update Cluster Pillar to [{pillar_cluster_table}] ===")
    ...
    def _download_pivot_ratio(self):
        ...
        end_date = np.max(self.period_list) + relativedelta(
            weeks=self.weeks_to_expire)
        start_date = np.min(self.period_list) - relativedelta(
            years=self.lookback)
        ...
        if self.ticker_list is not None:
            conditions.append(
                models.FactorPreprocessRatio.ticker.in_(self.ticker_list))
    ...
    def _calc_cluster(self, *args, ratio_df=None) -> Union[None, pd.DataFrame]:
        ...
        subset_end_date = testing_period + relativedelta(
            weeks=self.weeks_to_expire)
        subset_start_date = subset_end_date - relativedelta(years=self.lookback)
        df = ratio_df.loc[
            (ratio_df.index.get_level_values(
                "currency_code") == currency_code) &
            (ratio_df.index.get_level_values(
                "trading_day") <= subset_end_date.date()) &
            (ratio_df.index.get_level_values(
                "trading_day") > subset_start_date.date())]
