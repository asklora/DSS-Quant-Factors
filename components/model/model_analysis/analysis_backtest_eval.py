...
def save_plot_backtest_ret(result_all_comb, other_train_currency_col, pillar):
    """ Save Plot for backtest average ret """

    logger.debug(f'=== Save Plot for backtest average ret ===')
    result_all_comb = result_all_comb.copy()
    result_all_comb['other_train_currency'] = result_all_comb[other_train_currency_col].astype(str).agg('-'.join, axis=1)
    result_all_comb['group'] = result_all_comb[['group', 'group_code']].astype(str).agg('-'.join, axis=1)

    num_train_currency = len(result_all_comb['group'].unique())
    num_other_train_currency = len(result_all_comb['other_train_currency'].unique())

    # create figure for tests & train boxplot
    fig = plt.figure(figsize=(num_train_currency * 8, num_other_train_currency * 4), dpi=120, constrained_layout=True)
    k = 1
    for (other_train_currency, group), g in result_all_comb.groupby(['other_train_currency', 'group']):
        ax = fig.add_subplot(num_other_train_currency, num_train_currency, k)
        ...
        if (k % num_train_currency == 1) or (num_train_currency == 1):
            ax.set_ylabel(other_train_currency, fontsize=20)
        if k > (num_other_train_currency - 1) * num_train_currency:
            ax.set_xlabel(group, fontsize=20)
        if k == 1:
            plt.legend(['best', 'average', 'worse'])
        k += 1
    fig_name = f'#pred_{rank_pred.name_sql}_{pillar}.png'
    plt.suptitle(' - '.join(other_train_currency_col), fontsize=20)
