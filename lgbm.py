...
def lgbm_train(space):
    ...
    sql_result['feature_importance'] = to_list_importance(gbm, feature_names)
...
def to_sql_prediction(Y_test_pred):
    ...
    df = pd.DataFrame()
    df[global_vals.ticker_column] = test_id
    df['pred'] = Y_test_pred
    df['finish_timing'] = [sql_result['finish_timing']] * len(test_id)      # use finish time to distinguish dup pred
    return df

def to_list_importance(gbm, feature_names):
...
def HPOT(space, max_evals):
    ...
    with global_vals.engine.connect() as conn:  # write stock_pred for the best hyperopt records to sql
...
if __name__ == "__main__":
    ...
    testing_period_list=[last_test_date+relativedelta(days=1) - i*relativedelta(months=3)
                         - relativedelta(days=1) for i in range(0, args.backtest_period+1)]
    ...
    for group_code in ['industry', 'currency']:
        ...
        for testing_period in reversed(testing_period_list):
            ...
            load_data_params = {'backtest': backtest,  # last sample is not back testing
                                'qcut_q': args.qcut_q}

            # if 1==1:
            try:
                sample_set, cv, test_id, feature_names = data.split_all(testing_period, **load_data_params)  # load_data (class) STEP 3
