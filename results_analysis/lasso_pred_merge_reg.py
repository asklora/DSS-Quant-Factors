...
def download_stock_pred():
    ...
    with global_vals.engine_ali.connect() as conn:
        query = text(f"SELECT P.pred, P.actual_exact as actual, P.y_type, S.group_code, S.testing_period, CONCAT(S.alpha, '_',S.use_pca) as alpha FROM {global_vals.result_pred_table}_lasso P "
...
def download_stock_pred_multi():
    ''' download training history and training prediction from DB '''

    with global_vals.engine_ali.connect() as conn:
        query = text(f"SELECT P.*, S.name_sql, S.group_code, S.testing_period, S.alpha FROM {global_vals.result_pred_table}_lasso P "
                     f"INNER JOIN {global_vals.result_score_table}_lasso S ON S.finish_timing = P.finish_timing "
                     f"AND P.actual IS NOT NULL ORDER BY S.finish_timing")
        result_all = pd.read_sql(query, conn, chunksize=10000)       # download training history
        result_all = pd.concat(result_all, axis=0, ignore_index=True)
    global_vals.engine_ali.dispose()

    # remove duplicate samples from running twice when testing
    result_all = result_all.drop_duplicates(subset=['group_code', 'testing_period', 'y_type', 'alpha', 'name_sql'], keep='last')
    ...
    ret_dict = {}
    for name, g in result_all.groupby(['name_sql', 'group_code', 'testing_period', 'alpha']):
        ret_dict[name] = {}
        max_g = g.loc[g['pred']>g['pred'].quantile(q=2/3)]
        min_g = g.loc[g['pred']<g['pred'].quantile(q=1/3)]
        ret_dict[name]['max_factor'] = ','.join(list(max_g['y_type'].values))
