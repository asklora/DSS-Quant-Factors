...
params = ['n_estimators','max_depth','min_samples_split','min_samples_leaf','max_features','max_samples']
...
def download_from_score():
    with engine_ali.connect() as conn:
        query = text(f"select * "
                     f"FROM (SELECT DISTINCT *, max({matrix}_valid_mean) OVER (partition by group_code, testing_period, "
                     f"cv_number, y_type) as max_thing FROM {result_score_table}_rf_class WHERE name_sql = '{r_name}') t "
                     f"where {matrix}_valid_mean = max_thing")
        results = pd.read_sql(query, con=conn)  # read records from DB
    engine_ali.dispose()

    results[f'{matrix}_test_mean'] = results['accuracy_test'].apply(lambda x: x[1:-1]).str.split(',', expand=True).astype(float).mean(axis=1)
    return results
    ...
    for p in params:
        ...
        for name, g in df.groupby('group_code'):
            ...
            for i in range(len(max_scores)):
                ax.annotate(max_scores.iloc[i, -2].round(2), (max_scores.index[i][0], max_scores.index[i][1]),
                            fontsize=15)
        ...
        fig.savefig(f'tuning/rf_tuning_plot_{iter_name}_[{p}].png')
        plt.close()
...
def plot_scatter():
    ...
    for name, g in results.groupby(['group_code']):
        ...
        fig.tight_layout()
        fig.savefig(f'tuning/rf_tuning_plot_test_{iter_name}_{name}.png')
        plt.close()
