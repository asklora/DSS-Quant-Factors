...
def ali_migration_to_prod(migrate_tbl_lst):
    ...
    for t in migrate_tbl_lst:
        try:    # create new tables if not exist
            table = Table(t, metadata, autoload=True, autoload_with=global_vars.engine_ali)
            table.create(bind=global_vars.engine_ali_prod)
            print('---> Create table for: ', t)
        except Exception as e:
            print(t, e)

        with global_vars.engine_ali.connect() as conn_dev, global_vars.engine_ali_prod.connect() as conn_prod:
    ...
    prod_tbl_lst = get_table_name_list(global_vars.engine_ali_prod)
